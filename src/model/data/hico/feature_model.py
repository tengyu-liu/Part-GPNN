...
class VCOCO(torch.utils.data.Dataset):
    def __init__(self, root, input_imsize, transform, imageset):
        ...
        self.vcoco_feature_path = os.path.join(root, 'features_resnet')
        self.vcoco_path = os.path.join(root, '..', 'v-coco')
        self.imsize = input_imsize
        self.transform = transform

        self.coco = vu.load_coco(os.path.join(root, 'data'))
        self.vcoco_all = vu.load_vcoco('vcoco_{}'.format(imageset), os.path.join(root, 'data'))
        self.hoi_list = list()
        ...
        self.negative_num = 0 if imageset == 'test' else self.positive_num * 10
    ...
    def __getitem__(self, index):
        if index < self.positive_num:
            ...
            try:
                obj_boxes = np.load(os.path.join(self.vcoco_feature_path, '{}_obj_boxes.npy'.format(img_name)))
                part_boxes = np.load(os.path.join(self.vcoco_feature_path, '{}_part_boxes.npy'.format(img_name)))
                part_human_ids = np.load(os.path.join(self.vcoco_feature_path, '{}_part_human_id.npy'.format(img_name)))

                obj_i = random.randint(0, len(obj_boxes)-1)
                human_i = random.choice(list(set(part_human_ids)))

                roi = obj_boxes[obj_i, :]
                for part_i in range(len(part_boxes)):
                    if part_human_ids[part_i] == human_i:
                        roi = combine_box(roi, part_boxes[part_i, :])
            except:
        ...
        image_path = os.path.join(self.vcoco_path, 'coco/coco/images', '{}2014'.format(self.vcoco_imageset), img_name)
        ...
        roi_image = PIL.Image.fromarray(np.uint8(roi_image*255))
