...
def collate_fn_vcoco(batch):
    edge_features, node_features, part_human_id, adj_mat, node_labels, node_roles, obj_boxes, part_boxes, human_boxes, img_id, img_name, human_num, part_num, obj_num, obj_classes, part_classes, part_adj_mat, img_name = batch[0]
    part_human_id_batch = [part_human_id for (edge_features, node_features, part_human_id, adj_mat, node_labels, node_roles, obj_boxes, part_boxes, human_boxes, img_id, img_name, human_num, part_num, obj_num, obj_classes, part_classes, part_adj_mat, img_name) in batch]
    max_node_num = np.max(np.array([[node_features.shape[0]] for (edge_features, node_features, part_human_id, adj_mat, node_labels, node_roles, obj_boxes, part_boxes, human_boxes, img_id, img_name, human_num, part_num, obj_num, obj_classes, part_classes, part_adj_mat, img_name) in batch]))
    max_adj_num = np.max(np.array([[adj_mat.shape[0]] for (edge_features, node_features, part_human_id, adj_mat, node_labels, node_roles, obj_boxes, part_boxes, human_boxes, img_id, img_name, human_num, part_num, obj_num, obj_classes, part_classes, part_adj_mat, img_name) in batch]))
    # max_part_num = np.max(np.array([[part_adj_mat.shape[0]] for (edge_features, node_features, part_human_id, adj_mat, node_labels, node_roles, obj_boxes, part_boxes, human_boxes, img_id, img_name, human_num, part_num, obj_num, obj_classes, part_classes, part_adj_mat, img_name) in batch]))
    edge_feature_len = edge_features.shape[2]
    node_feature_len = node_features.shape[1]
    node_label_dim = node_labels.ndim
    ...
    edge_features_batch = np.zeros((len(batch), max_node_num, max_node_num, edge_feature_len))
    node_features_batch = np.zeros((len(batch), max_node_num, node_feature_len))
    adj_mat_batch = np.zeros((len(batch), max_adj_num, max_adj_num))
    # part_adj_mat_batch = np.zeros((len(batch), max_part_num, max_part_num))
    if node_label_dim > 1:
        node_labels_batch = np.zeros((len(batch), max_node_num, node_label_len))
    else:
        node_labels_batch = np.zeros((len(batch), max_node_num))
    ...
    img_names = list()
    img_ids = list()
    obj_boxes_batch = list()
    part_boxes_batch = list()
    human_boxes_batch = list()
    human_nums = list()
    part_nums = list()
    obj_nums = list()
    obj_classes_batch = list()
    part_classes_batch = list()

    for i, (edge_features, node_features, part_human_id, adj_mat, node_labels, node_roles, obj_boxes, part_boxes, human_boxes, img_id, img_name, human_num, part_num, obj_num, obj_classes, part_classes, part_adj_mat, img_name) in enumerate(batch):
        node_num = node_features.shape[0]
        adj_num = adj_mat.shape[0]
        edge_features_batch[i, :node_num, :node_num, :] = edge_features
        node_features_batch[i, :node_num, :] = node_features
        adj_mat_batch[i, :adj_num, :adj_num] = adj_mat
        # part_adj_mat_batch[i, :part_num, :part_num] = part_adj_mat
        if node_label_dim > 1:
            node_labels_batch[i, :adj_num, :] = node_labels
        else:
            node_labels_batch[i, :adj_num] = node_labels
        node_roles_batch[i, :adj_num, :] = node_roles
        img_names.append(img_name)
        img_ids.append(img_id)
        obj_boxes_batch.append(obj_boxes)
        part_boxes_batch.append(part_boxes)
        human_boxes_batch.append(human_boxes)
        human_nums.append(human_num)
        part_nums.append(part_num)
        obj_nums.append(obj_num)
        obj_classes_batch.append(obj_classes)
        part_classes_batch.append(part_classes)
    ...
    part_adj_mat_batch = None # torch.FloatTensor(part_adj_mat_batch)

    return edge_features_batch, node_features_batch, part_human_id_batch, adj_mat_batch, node_labels_batch, node_roles_batch, obj_boxes_batch, part_boxes_batch, human_boxes_batch, img_ids, img_names, human_nums, part_nums, obj_nums, obj_classes_batch, part_classes_batch, part_adj_mat_batch, img_names
