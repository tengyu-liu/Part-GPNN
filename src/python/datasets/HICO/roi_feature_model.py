...
class HICO(torch.utils.data.Dataset):
    def __init__(self, root, input_imsize, transform, imageset):
        ...
        bbox_file = os.path.join(root, 'anno_bbox.mat')
        bbox_anno = sio.loadmat(bbox_file)['bbox_{}'.format(imageset)]
        ...
        for img_i in range(bbox_anno.shape[1]):
            filename = bbox_anno[0,img_i][0][0]
            bbox_annotation = bbox_anno[0,img_i]
            for hoi_i in range(len(bbox_anno[0,img_i][2][0])):
                invis = bbox_anno[0,img_i][2][0][hoi_i][4][0,0]
                if invis == 1: continue
                action = metadata.hoi_to_action[bbox_anno[0,img_i][2][0][hoi_i][0][0,0]-1]
                bbox_h = bbox_anno[0,img_i][2][0][hoi_i][1]
                bbox_o = bbox_anno[0,img_i][2][0][hoi_i][2]
                h_idx = bbox_anno[0,img_i][2][0][hoi_i][3][0,0]
                o_idx = bbox_anno[0,img_i][2][0][hoi_i][3][0,1]
                ...
                data['img_files'].append(filename)
                data['bbxs'].append([x0_h,y0_h,x1_h,y1_h,x0_o,y0_o,x1_o,y1_o])
    ...
    def __getitem__(self, index):
        ...
        original_img = cv2.imread(image_path, mode='RGB')[:,:,[2,1,0]]
