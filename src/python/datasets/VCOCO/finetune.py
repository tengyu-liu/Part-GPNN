...
def main(args):
    ...
    criterion = torch.nn.CrossEntropyLoss(weight=torch.FloatTensor(metadata.action_class_weight)).cuda()
    ...
    train_transform = torchvision.transforms.Compose([
        ...
        torchvision.transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.15), 
        torchvision.transforms.RandomAffine(degrees=15, shear=15, resample=PIL.Image.BILINEAR),
    ...
    train_dataset = feature_model.VCOCO(args.data, input_imsize, train_transform, 'train')
    val_dataset = feature_model.VCOCO(args.data, input_imsize, train_transform, 'val')
    test_dataset = feature_model.VCOCO(args.data, input_imsize, test_transform, 'test')
